...
export default function PlanningUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = planningRecord
      ...
      : initialValues;
    setGiftForId(cleanValues.giftForId);
    setGiftFromId(cleanValues.giftFromId);
    setStatus(cleanValues.status);
    setWantToGetGifts(cleanValues.wantToGetGifts);
    setErrors({});
  };
  const [planningRecord, setPlanningRecord] = React.useState(planning);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Planning, idProp)
        : planning;
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Planning.copyOf(planningRecord, (updated) => {
