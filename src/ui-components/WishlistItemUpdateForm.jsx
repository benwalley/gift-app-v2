...
export default function WishlistItemUpdateForm(props) {
  const {
    id: idProp,
    wishlistItem,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = wishlistItemRecord
      ? { ...initialValues, ...wishlistItemRecord }
      : initialValues;
    ...
    setGottenBy(cleanValues.gottenBy ?? []);
    setCurrentGottenByValue("");
    setWantsToGet(cleanValues.wantsToGet ?? []);
    setCurrentWantsToGetValue("");
    setCustom(cleanValues.custom);
    setIsPublic(cleanValues.isPublic);
    setCreatedById(cleanValues.createdById);
    setSeenBy(cleanValues.seenBy ?? []);
    setCurrentSeenByValue("");
    setErrors({});
  };
  const [wishlistItemRecord, setWishlistItemRecord] =
    React.useState(wishlistItem);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(WishlistItem, idProp)
        : wishlistItem;
      setWishlistItemRecord(record);
    };
    queryData();
  }, [idProp, wishlistItem]);
  React.useEffect(resetStateValues, [wishlistItemRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            WishlistItem.copyOf(wishlistItemRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "WishlistItemUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || wishlistItem)}
