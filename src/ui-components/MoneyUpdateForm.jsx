...
export default function MoneyUpdateForm(props) {
  const {
    id: idProp,
    money,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = moneyRecord
      ? { ...initialValues, ...moneyRecord }
      : initialValues;
    setOwedFromName(cleanValues.owedFromName);
    setOwedToName(cleanValues.owedToName);
    setAmount(cleanValues.amount);
    setOwnerId(cleanValues.ownerId);
    setNote(cleanValues.note);
    setErrors({});
  };
  const [moneyRecord, setMoneyRecord] = React.useState(money);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Money, idProp) : money;
      setMoneyRecord(record);
    };
    queryData();
  }, [idProp, money]);
  React.useEffect(resetStateValues, [moneyRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Money.copyOf(moneyRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MoneyUpdateForm")}
      {...rest}
    >
      <TextField
